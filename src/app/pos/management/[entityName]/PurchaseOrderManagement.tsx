...
import { FaPlus, FaFileInvoice, FaBoxOpen, FaSearch, FaCheckCircle, FaClock, FaExclamationTriangle } from "react-icons/fa";
import { getPurchaseOrders, getGoodsReceipts, getSuppliers, getSites } from "@/lib/api";
...
export default function PurchaseOrderManagement() {
    ...
    const fetchData = async () => {
        try {
            ...
            const enrichedOrders = orders.map(order => ({
                ...order,
                supplier_name: supplierList.find(s => s.id === order.supplier_id)?.name || 'Unknown',
                site_name: siteList.find(s => s.id === order.site_id)?.name || 'Unknown'
            }));
    ...
    const getStatusIcon = (status: string) => {
        switch (status) {
            case 'draft': return FaFileInvoice;
            case 'pending-approval': return FaClock;
            case 'approved': return FaCheckCircle;
            case 'ordered': return FaFileInvoice;
            case 'partially-received': return FaExclamationTriangle;
            case 'received': return FaBoxOpen;
    ...
    const stats = {
        ...
        receiving: getStatusCount('partially-received') + getStatusCount('received')
    };
    ...
    return (
        <Box p={6}>
            <Flex justifyContent="space-between" alignItems="center" mb={6}>
                <Heading as="h1" size="xl">Purchase Order Management</Heading>
                <Button
                    leftIcon={<FaPlus />}
                    colorScheme="blue"
            ...
            <Tabs variant="enclosed" onChange={setActiveTab}>
                <TabList>
                    ...
                    <Tab>
                        <HStack>
                            <Icon as={getStatusIcon('partially-received')} color="yellow.500" />
                            <Text>Receiving</Text>
