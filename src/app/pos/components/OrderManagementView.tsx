...
const OrderManagementView: React.FC<OrderManagementViewProps> = ({
  ...
  const handleStatusChange = async (
    ...
    try {
      ...
      console.log(
        `LOG: Order #${order.id} status changed to ${newStatus} in Order Management.`
      );
    } catch (error) {
      toast({
        title: "Error",
      ...
      console.error(
        `ERROR: Failed to update order #${order.id} status to ${newStatus}.`,
        error
      );
    }
  };
  ...
  const OrderCard: React.FC<{ order: Order }> = ({ order }) => (
    ...
    >
      <Flex align="center" mb={2}>
        <Text fontWeight="bold" fontSize="md" color="var(--dark-gray-text)">
          Order #{order.id}
        </Text>
        <Spacer />
        <Badge
      ...
      <HStack mt={3} spacing={2} justifyContent="flex-end">
        <Button size="xs" colorScheme="blue" onClick={() => onLoadOrder(order)}>
          Load
        </Button>
        {order.status === "new" && (
          <Button
