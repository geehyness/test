...
export default function POSDashboardPage() {
  ...
  const {
    ...
    processOrderPayment,
    setCurrentOrder, // <--- THIS LINE WAS ADDED/UNCOMMENTED
  } = usePOSStore();
  ...
  const handleApplyDiscount = async () => {
    if (discountCode === "SAVE10") {
      ...
      applyDiscountToOrder(5.0, "fixed");
      toast({
        title: "Discount Applied",
  ...
  const handleSelectNewOrderTable = (tableId: string | null) => {
    ...
    onNewOrderTableModalClose(); // Close table modal

    // Calculate subtotal, tax, total for the new order
    const subtotal_amount = tempNewOrderItems.reduce(
      (sum, item) => sum + (item.sub_total || 0),
      0
    );
    // Assuming a fixed tax rate for simplicity, or fetch from settings
    const tax_percentage = 0.15; // 15% tax
    const tax_amount = subtotal_amount * tax_percentage;
    const total_amount = subtotal_amount + tax_amount;

    // Create a mock new order object
    const newOrder: Order = {
      id: "", // Will be generated by fetchData on POST
      tenant_id: "tenant-231", // Default tenant ID
      table_id: tableId,
      customer_id: null, // Can be added later if needed
      total_amount: total_amount,
      status: "new", // Initial status
      notes: "",
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
      items: tempNewOrderItems,
      subtotal_amount: subtotal_amount,
      tax_amount: tax_amount,
      discount_amount: 0,
      employee_id: "emp-101", // Default employee ID
      order_type: tableId ? "dine-in" : "takeaway",
    };

    setCurrentOrder(newOrder); // Set the new order as the current order in the store
            ...
            <Flex h="100%" gap={6}>
              ...
              >
                ...
                >
                  <TabList mb={4}>
                    <Tab
                      _selected={{ bg: "var(--primary-green)", color: "white" }}
                      fontSize="md"
                      fontWeight="semibold"
                    >
          ...
          >
            <VStack spacing={4} align="stretch" py={10}>
              ...
              <Text textAlign="center" color="var(--medium-gray-text)">
                This section will provide detailed insights into sales, orders,
